function fish_clipboard_copy -d "Copy to clipboard with remote support over ssh with OSC52"
    set -l cmdline
    if isatty stdin
    ...
    if type -q base64
        ...
        if set -q TMUX
            ...
            if test -w $tmux_tty
                ...
                echo "fish_clipboard_copy: could not write to terminal behind tmux, maybe switched user ?"
            end
        end
    end
end
